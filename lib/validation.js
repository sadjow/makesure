...
validation.execute = function(obj, callback) {
  ...
  setImmediate(function(){
    ...
    if(typeof self._validation.execute == 'function') {
      for(var i = 0; i < self._attrs.length; i++) {
        var attrName = self._attrs[i];
        fns.push(function(cb) {
          var cba = function(errors, obj) {
            errors.__parentAttr__ = attrName;
            cb(null, errors);
          };
          self._validation(obj[attrName], cba);
        });
      }
      ...
      async.parallel(fns, function(err, results){
        ...
        for(var i = 0; i < results.length; i++) {
          var r = results[i];
          result[r.__parentAttr__] = merge.recursive(result[r.__parentAttr__], results[i].error);
        }
        callback(null, { error:{  attrs: result }});
      });
